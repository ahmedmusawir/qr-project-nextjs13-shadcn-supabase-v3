...
const CountdownTimer = ({ delaySeconds, onComplete }: CountdownTimerProps) => {
  ...
  // Fetch the current syncStatus from Zustand store
  ...
  useEffect(() => {
    ...
    // Only update Zustand with the complete SyncStatus structure
    if (syncStatus) {
      updateSyncStatus({ ...syncStatus, delay_in_sec: timeLeft });
    }

    return () => clearInterval(intervalId); // Cleanup interval on unmount
  }, [delaySeconds, onComplete, timeLeft, syncStatus, updateSyncStatus]);
